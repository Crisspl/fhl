...
#	 This file was modified by author of FHL (https://github.com/Crisspl/FHL) and slightly differs from the original one available at https://github.com/skaslev/gl3w
#
...
if not os.path.exists(os.path.join(args.root, 'glcorearb.h')):
    print('Downloading glcorearb.h to {0}...'.format(os.path.join(args.root, 'glcorearb.h')))
    web = urllib2.urlopen('http://www.opengl.org/registry/api/GL/glcorearb.h')
    with open(os.path.join(args.root, 'glcorearb.h'), 'wb') as f:
...
# Generate gl3w.h
print('Generating gl3w.h in {0}...'.format(args.root))
...
FHL_API extern union GL3WProcs gl3wProcs;
...
# Generate gl3w.c
print('Generating gl3w.c in {0}...'.format(args.root))
with open(os.path.join(args.root, 'gl3w.c'), 'wb') as f:
    write(f, UNLICENSE)
    write(f, r'''#include <FHL/GL/gl3w.h>
#include <stdlib.h>
...
FHL_API union GL3WProcs gl3wProcs;
