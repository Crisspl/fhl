...
{
	std::map<Buffer::Target, GLuint> Buffer::s_bonds;
	...
	Buffer::~Buffer()
	{
		for (auto t : m_targets)
			if (s_bonds[t] == m_id) s_bonds[t] = 0;
		glDeleteBuffers(1, &m_id);
	}

	void Buffer::bind(Target _target) const
	{
		if (s_bonds[_target] == m_id) return;
